...
const Logo = () => (
  <a href="/" className="flex items-center gap-3 group" aria-label="Home">
    <div className="relative w-11 h-11 flex items-center justify-center rounded-xl overflow-hidden logo-box">
      {/* Animated gradient border */}
      <div className="absolute inset-0 rounded-xl logo-border-anim" />
      <div
      ...
      >
        <span className="text-xl logo-bolt">⚡</span>
      </div>
    </div>
...
export default function Navbar() {
  ...
  useEffect(() => {
    const handler = (e) => { if (dropRef.current && !dropRef.current.contains(e.target)) setDropdown(null); };
  ...
  const isActive = (name) => activeSection === name || (name === "Home" && activeSection === "Home");
    ...
    <>
      <style>{`
        ...
        .logo-border-anim {
          ...
          animation: logoBorderSpin 3s linear infinite;
        }
        ...
        .active-dot {
          ...
          animation: dotPop 0.3s cubic-bezier(.22,1,.36,1) both;
        }
        @keyframes dotPop {
        ...
        /* ── Dropdown ── */
        .dropdown-panel {
          animation: dropDown 0.25s cubic-bezier(.22,1,.36,1) both;
        ...
        /* ── Mobile menu slide ── */
        .mobile-menu {
          animation: slideDown 0.3s cubic-bezier(.22,1,.36,1) both;
        }
        @keyframes slideDown {
      ...
      <nav
        className={`navbar-glass sticky top-0 z-50 w-full ${scrolled ? "scrolled" : ""}`}
        style={{
      ...
      >
        ...
        <div className="hidden lg:flex items-center justify-between max-w-7xl mx-auto px-6 py-4">
              ...
              item.submenu ? (
                <div key={item.name} className="relative">
                  <button
                    ...
                    className={`nav-link ${isActive(item.name) ? "active" : ""} flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold transition-colors duration-200 ${isActive(item.name) ? "text-white" : "text-slate-400 hover:text-white"}`}
                  >
                    {item.name}
                    <Chevron open={dropdown === item.name} />
                    {isActive(item.name) && <span className="active-dot" />}
                  </button>

                  {dropdown === item.name && (
                    <div
                      className="dropdown-panel absolute top-full left-0 mt-3 w-52 rounded-xl border border-white/10 overflow-hidden"
                      style={{
                    ...
                    >
                      ...
                      {item.submenu.map((sub, i) => (
                        <button
                          ...
                          className="dropdown-item w-full text-left px-4 py-3 text-sm text-slate-300 hover:text-white hover:bg-white/5 flex items-center gap-2 group"
                        >
                          <span
                            className="w-1.5 h-1.5 rounded-full flex-shrink-0 transition-all duration-200 group-hover:scale-125"
                            style={{ background: i % 2 === 0 ? "#6366f1" : "#06b6d4", boxShadow: `0 0 6px ${i % 2 === 0 ? "#6366f1" : "#06b6d4"}` }}
                          />
                          {sub.name}
                        </button>
                      ))}
                ...
                <button
                  ...
                  className={`nav-link relative ${isActive(item.name) ? "active" : ""} px-4 py-2 rounded-lg text-sm font-semibold transition-colors duration-200 ${
                    item.name === "Projects"
          ...
          <button
            ...
            className="wa-btn flex items-center gap-2.5 px-5 py-2.5 rounded-xl text-sm font-bold text-white"
            style={{
        ...
        <div className="lg:hidden px-4 py-3.5">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="ham p-2 rounded-lg border border-white/10"
              style={{ background: "rgba(255,255,255,0.05)" }}
              aria-label="Toggle Menu"
              aria-expanded={mobileOpen}
              {...{ className: `ham ${mobileOpen ? "open" : ""} p-2 rounded-lg border border-white/10` }}
            >
              <span /><span /><span />
            </button>
          </div>
          ...
          {mobileOpen && (
            <div
              className="mobile-menu mt-3 rounded-2xl border border-white/10 overflow-hidden"
              style={{
            ...
            >
              ...
              <div className="px-3 py-3 flex flex-col gap-1">
                  ...
                  item.submenu ? (
                    ...
                    >
                      <span
                        ...
                        style={{
                          background: item.name === "Projects" ? "#818cf8" :
                ...
                <button
                  onClick={() => window.open(`https://wa.me/918948681079`, "_blank")}
                  className="wa-btn mt-2 flex items-center justify-center gap-2.5 w-full py-3.5 rounded-xl text-sm font-bold text-white"
                  style={{
                    background: "linear-gradient(135deg,#16a34a,#22c55e)",
                    boxShadow: "0 4px 16px rgba(34,197,94,0.3)",
                  }}
